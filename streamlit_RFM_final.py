...
def upload_file(uploaded_file_1,uploaded_file_2):
    ...
    if uploaded_file_1 is not None:
        new_df2=load_txt(uploaded_file_1)
        new_df2 = read_data(new_df2)
        new_df2= create_RFM(new_df2)
        st.write("Thanks for your RAW file")
        return new_df2
    if uploaded_file_2 is not None:
        new_df2=load_csv(uploaded_file_2)
...
@st.cache_data(max_entries=1000)
def load_csv(file):
    csv=pd.read_csv(file,index_col='customer_ID')
    return csv
...
Label_model_new = load_dill('functions/Label_model_new.pkl')
...
new_df1=new_df[['Recency','Frequency','Monetary']].copy()
    ...
    if type=="Upload":
        ...
        if st.button('👉Start👈'):
            if (uploaded_file_1 is None) and (uploaded_file_2 is None):
                ...
                new_df3=pd.concat([new_df2,new_df1],ignore_index=True)
                lines = preprocess(new_df3)
                PCA_components = PCA_model(lines)
                lines=PCA_components.iloc[:,:1]
                lines=lines.iloc[range(len(new_df2))]
                st.write("New data prediction:")
                y_pred_new = model.predict(lines)
                rfm_rfm_k3 = Label_model_new(new_df2,y_pred_new)
    ...
    if type=="Input_new_customer":
        ...
        if st.button('👉Start👈'):
            if (Recency!="")&(Frequency!=0)&(Monetary!=0):
                ...
                new_df2=pd.concat([new_df2,data_input],ignore_index=True)
                lines = preprocess(new_df2)
                PCA_components = PCA_model(lines)
                lines=PCA_components.iloc[:,:1]
                lines=lines.iloc[-1]
                st.write("New Customer Prediction:")
                y_pred_new=model.predict([lines])
                rfm_rfm = Label_model_new(data_input,y_pred_new)
                st.dataframe(rfm_rfm)
